...
export const addComment = createAsyncThunk('comments/addComment', async ({ postId, content, author }) => {


  try {
    const { data } = await instance.post(`/posts/${postId}/comments`, { postId, content, author });
...
export const deleteComment = createAsyncThunk("comments/deleteComment", async ({postId, _id}) => {
	try {
